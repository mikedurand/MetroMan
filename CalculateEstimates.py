...
def CalculateEstimates(C,D,Obs,Prior,DAll,AllObs,nOpt):
    ...
    E=Estimates(D,DAll)
    E.A0hat=mean(C.thetaA0[:,C.Nburn:],1)    
    E.CA0=cov(C.thetaA0[:,C.Nburn:])
    E.stdA0Post=sqrt(diagonal(E.CA0)) 
    
    E.nahat=mean(C.thetana[:,C.Nburn:],1)    
    E.Cna=cov(C.thetana[:,C.Nburn:])    
    E.stdnaPost=sqrt(diagonal(E.Cna)) 

    E.x1hat=mean(C.thetax1[:,C.Nburn:],1)    
    E.Cx1=cov(C.thetax1[:,C.Nburn:])    
    ...
    #5) all estimates
    for i in range(0,D.nR):
        E.AllQ[i,:]=1/E.nhatAll[i,:]*( (E.A0hat[i]-AllObs.A0Shift[i])*ones([1,DAll.nt])+AllObs.dA[i,:] )**(5/3) * AllObs.w[i,:]**(-2/3) * AllObs.S[i,:]**0.5
