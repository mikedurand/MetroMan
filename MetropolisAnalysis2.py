...
#   originally translated from Matlab to Python 2.x ~2010, by Yeosang Yoon, now at NASA GSFC
#   translated to Python 3 by Mike, in 2020
#   to be updated with the latest version of MetroMan, Summer 2020
...
for i in range(0,N):
    ...
    Like[0,i]=exp(fu)
toc=time.process_time(); print('Computing Time: %.2fs' %(toc-tic))
...
plt.figure(2);
p1=plt.errorbar(xkm/1000,A0hat,stdA0Post,linewidth=2,color='r') 
p2=plt.errorbar(xkm/1000,meanA0.reshape(size(meanA0),),stdA0,linewidth=2,color='g')
p3=plt.errorbar(xkm/1000,A0true.T,linewidth=2) 
plt.xlabel('Flow distance, km')
plt.ylabel('Cross-sectional area, m^2')
plt.legend((p1,p2,p3),('Estimate','Prior','True'))     
#plt.show()
...
plt.figure(5)
plt.plot(t[0,1:nt],qhatm)
plt.plot(t[0,1:nt],qtruem,'--')
...
plt.figure(7)
plt.plot(list(range(1,N+1)),Like.T)

plt.figure(8)
for i in range(0,nR*(nt-1)):
    plt.subplot(3,6,i+1)
    plt.hist(thetaq[i,Nburn+1:N])
    a=plt.axis();
